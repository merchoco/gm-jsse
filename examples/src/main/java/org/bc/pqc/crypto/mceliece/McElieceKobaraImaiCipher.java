...
public class McElieceKobaraImaiCipher implements MessageEncryptor {
   ...
   private void initCipherEncrypt(McElieceCCA2PublicKeyParameters var1) {
      this.messDigest = var1.getParameters().getDigest();
      this.n = var1.getN();
      this.k = var1.getK();
      this.t = var1.getT();
   }
   ...
   public byte[] messageEncrypt(byte[] var1) throws Exception {
   ...
   public byte[] messageDecrypt(byte[] var1) throws Exception {
      int var2 = this.n >> 3;
      if (var1.length < var2) {
         ...
         byte[] var7;
         if (var5 > 0) {
            byte[][] var8 = ByteUtils.split(var1, var5);
            var6 = var8[0];
            var7 = var8[1];
         } else {
            var6 = new byte[0];
         ...
         GF2Vector var24 = GF2Vector.OS2VP(this.n, var7);
         ...
         byte[] var13 = ByteUtils.concatenate(var6, var12);
