...
public class JDKDSAPublicKey implements DSAPublicKey {
   ...
   public byte[] getEncoded() {
      try {
         return this.dsaSpec == null ? (new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa), new DERInteger(this.y))).getEncoded("DER") : (new SubjectPublicKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa, new DSAParameter(this.dsaSpec.getP(), this.dsaSpec.getQ(), this.dsaSpec.getG())), new DERInteger(this.y))).getEncoded("DER");
