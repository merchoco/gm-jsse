...
public class JCEDHPrivateKey implements DHPrivateKey, PKCS12BagAttributeCarrier {
   ...
   JCEDHPrivateKey(PrivateKeyInfo var1) throws IOException {
      ...
      ASN1Integer var3 = DERInteger.getInstance(var1.parsePrivateKey());
   ...
   public byte[] getEncoded() {
      try {
         if (this.info != null) {
            ...
            PrivateKeyInfo var1 = new PrivateKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.dhKeyAgreement, new DHParameter(this.dhSpec.getP(), this.dhSpec.getG(), this.dhSpec.getL())), new DERInteger(this.getX()));
