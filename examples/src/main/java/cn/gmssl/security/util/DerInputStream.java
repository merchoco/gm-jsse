...
public class DerInputStream {
   ...
   private void init(byte[] var1, int var2, int var3) throws IOException {
      if (var2 + 2 <= var1.length && var2 + var3 <= var1.length) {
         if (DerIndefLenConverter.isIndefinite(var1[var2 + 1])) {
            ...
            DerIndefLenConverter var5 = new DerIndefLenConverter();
            this.buffer = new DerInputBuffer(var5.convert(var4));
         } else {
   ...
   protected DerValue[] readVector(int var1) throws IOException {
      ...
      if (var4 == -1) {
         ...
         this.buffer = new DerInputBuffer(var9.convert(var7));
      ...
      if (var4 == 0) {
         ...
         do {
            DerValue var11 = new DerValue(var2.buffer);
